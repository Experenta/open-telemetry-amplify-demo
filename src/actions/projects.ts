...
const projectCreationPhaseCounter = meter.createCounter("projects.creation.phase", {
...
const projectProcessingTime = meter.createHistogram("projects.processing.time", {
...
function recordStartEvent(
    ...
    context: { projectId?: string; projectStatus?: ProjectStatus; phaseCounter: any; }
) {
    const commonAttrs = getCommonAttributes(context);
    const actionAttrs = getActionAttributes(operationName, operationName.includes("create") ? "create" : operationName.includes("read") ? "read" : operationName.includes("update") ? "update" : "delete");
...
function recordCompleteEvent(
    span: Span,
    operationName: string,
    context: { projectId?: string; projectStatus?: ProjectStatus; phaseCounter: any; startTime: number; attributes?: Record<string, any> }
) {
    ...
    projectEventsCounter.add(1, {
        "event.name": `project.${operationName.includes("create") ? "create" : operationName.includes("read") ? "fetch" : operationName.includes("update") ? "update" : "delete"}.completed`,
        "operation": operationName,
        "project.id": context.projectId || "none",
    });
...
function recordErrorEvent(
    ...
    operationName: string,
    error: unknown,
    context: { projectId?: string; projectStatus?: ProjectStatus; phaseCounter: any; errorType: "database" | "runtime" }
) {
    ...
    projectEventsCounter.add(1, {
        "event.name": `project.${operationName.includes("create") ? "create" : operationName.includes("read") ? "fetch" : operationName.includes("update") ? "update" : "delete"}.exception`,
        "operation": operationName,
        "project.id": context.projectId || "none",
    });
    ...
    return await tracer.startActiveSpan(
        ...
        async (span) => {
            ...
            try {
                ...
                if (errors) {
                    recordErrorEvent(span, "getProjects", new Error(JSON.stringify(errors)), {
                        phaseCounter: projectFetchPhaseCounter,
                        errorType: "database",
                    });
                ...
                const statusCounts = projectsList.reduce((acc: any, p: any) => {
    ...
    return await tracer.startActiveSpan(
        ...
        async (span) => {
            try {
                ...
                const actionAttrs = getActionAttributes("getProjectById", "read");
                ...
                const { data: project, errors } =
                    await cookieBasedClient.models.Project.get(
                        { id },
                        { selectionSet: ["id", "name", "description", "status", "createdAt", "updatedAt"] }
                        

                    );
                
                if (errors) {
                    recordErrorEvent(span, "getProjectById", new Error(JSON.stringify(errors)), {
                        projectId: id,
                ...
                if (!project) {
                    ...
                    span.setStatus({ code: SpanStatusCode.ERROR, message: "Project not found" });
                    span.setAttributes({
                        "operation.status": "failed",
...
export async function createProject(formData: FormData) {
    ...
    return await tracer.startActiveSpan("projects.createProject",
        async (span) => {
            const eventStartTime = Date.now();
            ...
            try {
                ...
                if (errors) {
                    recordErrorEvent(span, "createProject", new Error(JSON.stringify(errors)), {
                        projectStatus: status,
                        phaseCounter: projectCreationPhaseCounter,
                        errorType: "database",
                    });
...
export async function updateProject(id: string, formData: FormData) {
    ...
    return await tracer.startActiveSpan("projects.updateProject",
        async (span) => {
            const eventStartTime = Date.now();
            ...
            try {
                ...
                if (errors) {
                    recordErrorEvent(span, "updateProject", new Error(JSON.stringify(errors)), {
                        projectId: id,
                        projectStatus: status,
                        phaseCounter: projectUpdatePhaseCounter,
                        errorType: "database",
...
export async function deleteProject(id: string) {
    ...
    return await tracer.startActiveSpan("projects.deleteProject",
        async (span) => {
            try {
                // Aquí no tenemos el status del proyecto, solo el ID
                const commonAttrs = getCommonAttributes({ projectId: id });
                const actionAttrs = getActionAttributes("deleteProject", "delete");
                ...
                const { errors } = await cookieBasedClient.models.Project.delete({ id });

                if (errors) {
                    recordErrorEvent(span, "deleteProject", new Error(JSON.stringify(errors)), {
                        projectId: id,
